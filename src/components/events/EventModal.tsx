...
export default function EventModal({ event, onClose, isLoading, error }: EventModalProps) {
  ...
  if (isLoading) {
    return (
      <EventModalSkeleton onClose={onClose} />
    );
  }
  ...
  function getEventUrl(eventURL: string | null, businessURL: string) {
      if (eventURL) {
  ...
  function handleOnShareClick(event: Event) {
    shareEvent(event);
